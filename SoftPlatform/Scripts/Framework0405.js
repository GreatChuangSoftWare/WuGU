...
//    str += '<div class="modal-header">';
//    str += '<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>';
//    str += '<h4 class="modal-title">顾客--添加</h4>';
//    str += '</div>';
//    str += '<div class="alert  alert-success messprompt hide" role="alert">';
//    str += '<span class="glyphicon glyphicon-ok-circle"><span>提示：</span>';
//    str += '<span class="messpromptmess">如果要隐藏请在父元素增加class=“hide”</span>';
//    str += '</div>';
//    str += '<div id="popupContent">';
//    str += '<div class="modal-body clearfix">';
//    str += '<form>';
...
//    str += '<button type="button" class="btn btn-info btn-primary">确定</button>';
//    str += '<button type="button" class="btn btn-Close">添加并保存</button>';
//    str += '</div>';
//    str += '</div>';
//    str += '</div>';
//    str += '</div>';
...
Framework.Tool.FwRowPopup = function (currobj) {
    ...
    if (btnbehavior == 11)//界面取数
    {
        ...
        for (att in tbheadsobjs) {
            ...
            strMess += "<li class='col-lg-" + colcopies + "'>";
            strMess += "    <div class='form-group'>";
            strMess += "        <label class='col-lg-4 control-label'>" + tbheadsobjs[att] + "</label>";
            strMess += "        <div class='col-lg-8'>";
            strMess += "        <label class='col-lg-4 control-label'>" + rowItem[att] + "</label>";
            //strMess += rowItem[att];
            strMess += "        </div>";
            strMess += "    </div>";
            strMess += "</li>";
        ...
        var params = {
            ...
            onConfirm: function (popup, popupbtn) {
                ...
                else if (fun == 21)//提交替换：2：posturl      rowurl
                {
    ...
    else if (btnbehavior == 12)//Get请求
    {
        var geturl = currobj.data("geturl");
        var prepname = "Item." + pkname;
        var data = {};
        data[prepname] = pkval;
        ...
        Framework.Ajax.GetView(data, geturl, function (result) {
            ...
            var params = {
                ...
                onConfirm: function (popup, currobj) {
                    fun = currobj.data("fun");
                    if (fun == 22)//确认(编辑)并关闭=>替换行，表单提交
                    {
                        var posturl = currobj.data("posturl");
                        //(1)验证数据
                        if (!Framework.Form.Validates(popup))
                        ...
                        Framework.Ajax.PostJson(data, posturl, function (results) {
                            ...
                            var rowurl = currobj.data("rowurl");
                            var pkval = popup.find("#Item_" + pkname).val();
                            var prepname = "Item." + pkname;
                            var data = {};
                            data[prepname] = pkval;
                            Framework.Ajax.GetView(data, rowurl, function (results) {
                    ...
                    else if (fun == 23)//确认并下一条，不同功能有不同下一条
                    {
                        var posturl = currobj.data("posturl");
                        //(1)验证数据
                        if (!Framework.Form.Validates(popup))
                        ...
                        Framework.Ajax.PostJson(data, posturl, function (results) {
                            ...
                            var nexturl = currobj.data("nexturl");
                            var pkval = popup.find("#Item_" + pkname).val();
                            var prepname = "Item." + pkname;
                            var data = {};
                            data[prepname] = pkval;

                            Framework.Ajax.GetView(data, nexturl, function (results) {
                                popup.find("#popupContent").html(results);
                                Framework.Tool.PopupEvenBind();
                            });
                        });
                    }
                    else if (fun == 24)//上一条，不同功能有不同下一条
                    {
                        var beforeurl = currobj.data("beforeurl");
                        var pkval = popup.find("#Item_" + pkname).val();
                        var prepname = "Item." + pkname;
                        var data = {};
                        data[prepname] = pkval;
                        Framework.Ajax.GetView(data, beforeurl, function (results) {
                            //str += '<div id="popupContent">';
                            popup.find("#popupContent").html(results);
                            //事件绑定
                            Framework.Tool.PopupEvenBind();
...
//                {
//                }
//                else if (fun == 2)//提交替换：2：posturl      rowurl
//                {
//                    var posturl = obj.data("posturl");
//                    var prepname = "Item." + pkname;
//                    var data = {};
...
//                    });
//                }
//                else if (fun == 3)//后台删除，删除table中的行，不刷新
//                {
//                    ////删除不刷新页面：3或4：posturl
//                    var posturl = currobj.data("posturl");
//                    var prepname = "Item." + pkname;
...
//                        });
//                    }
//                    else if (fun == 101)//确认并关闭=>替换行，确认并关闭=>替换行
//                    {
//                        ////删除不刷新页面：3或4：posturl
//                        //var posturl = currobj.data("posturl");
//                        //var prepname = "Item." + pkname;
...
//                        });
//                    }
//                    else if (fun == 102)//确认并下一条，不同功能有不同下一条
//                    {
//                        var posturl = currobj.data("posturl");
//                        var pkval = popup.find("#Item_" + pkname).val();
//                        var prepname = "Item." + pkname;
...
//                        });
//                    }
//                    else if (fun == 103)//上一条，不同功能有不同下一条
//                    {
//                        var beforeurl = currobj.data("beforeurl");
//                        var pkval = popup.find("#Item_" + pkname).val();
//                        var prepname = "Item." + pkname;
...
//                        });
//                    }
//                    else if (fun == 104)//下一条?不同功能有不同下一条
//                    {
//                        var nexturl = currobj.data("nexturl");
//                        var pkval = popup.find("#Item_" + pkname).val();
//                        var prepname = "Item." + pkname;
