...
Import('Pre_UserRoless');

(function (Pre_UserRoless) {

    Pre_UserRoless.Init = function () {

        var trPre_UserRolessDataFieldMap = new Hashtable();
        trPre_UserRolessDataFieldMap.add('pre_userrolesid', 'Pre_UserRolesID');
        trPre_UserRolessDataFieldMap.add('pre_usersid', 'Pre_UsersID');
        trPre_UserRolessDataFieldMap.add('pre_rolesid', 'Pre_RolesID');


        //////////////////快速：查询事件/////////////////////
        $('#mainPage #btn-Search').click(function () {
            $('#searchType').val('0');
            Pre_UserRoless.Search({});
        });
        ...
        ////////////////查询：pageIndex：当前页//////////////
        Pre_UserRoless.Search = function (params) {
            var data = {};
            //(1)找到查询条件：快速查询、高级查询
        ...
        $(document).delegate('#mainPage #tbbody .btn-Detail', 'click', function (e) {
            ...
            var data = Framework.Tool.GetTbRowData($(this), trPre_UserRolesDataFieldMap);
            Framework.Ajax.GetView(data, 'Pre_UserRoles/Detail', function (result) {
                var params = {
                    title: 'XXX详情',
                    message: result,
        ...
        $('#mainPage #toolbarbtn .btn-Add').click(function (e) {
            Framework.Ajax.GetView({}, 'Pre_UserRoles/Edit', function (result) {
                var params = {
                    title: '添加XX信息',
                    message: result,
                    onConfirm: function (popup, event) {
                        if (Framework.Form.Validates(popup)) {
                            ...
                            Framework.Ajax.PostJson(data, 'Pre_UserRoles/Edit', function (results) {
                                //错误处理
                                ...
                                //在列表中添加行
                                Framework.Ajax.GetView({ 'Item.Pre_UserRolesID': data.Item.NoticeID }, 'Pre_UserRoles/TableRow', function (results) {
                                    //错误处理
                                    if (results.Data.RespAttachInfo.bError) {
                                        //显示错误
                                        var strerrors = Framework.UI.Behavior.ErrorHandling(results);
        ...
        $(document).delegate('#mainPage #tbbody  .btn-Edit', 'click', function (e) {
            var tr = $(this).closest('tr');
            var data = Framework.Tool.GetTbRowData($(this), trPre_UserRolesDataFieldMap);
            Framework.Ajax.GetView(data, 'Pre_UserRoles/Edit', function (result) {
                var params = {
                    title: '编辑XX信息',
                    message: result,
                    onConfirm: function (popup, event) {
                        if (Framework.Form.Validates(popup)) {
                            ...
                            Framework.Ajax.PostJson(data, 'Pre_UserRoles/Edit', function (results) {
                                ...
                                var data = results.Data;
                                //更新列表行
                                Framework.Ajax.GetView({ 'Item.Pre_UserRolesID': data.Item.Pre_UserRolesID }, 'Pre_UserRoles/TableRow', function (results) {
                                    if (results.Data.RespAttachInfo.bError) {
                                        //显示错误
                                        var strerrors = Framework.UI.Behavior.ErrorHandling(results);
                                        if (strerrors.length > 0)
            ...
            var params = {
                ...
                onConfirm: function (popup) {
                    ...
                    $('#mainPage #tbbody  .jq-checkall-item:checked').each(function () {
                        var tr = $(this).closest('tr');
                        var rowItem = tr.data();
                        var obj = Framework.Tool.GetTbRowData($(this), trPre_UserRolessDataFieldMap);

                        Framework.Ajax.PostJsonAsync(obj, 'Pre_UserRoles/Delete',
                            function (result) {
        ...
        $(document).delegate('#mainPage #tbbody .btn-Delete', 'click', function (e) {
            var entityobj = Framework.Tool.GetTbRowData($(this), trPre_UserRolessDataFieldMap);
            var tr = $(this).closest('tr');
            var rowItem = tr.data();
            ...
            var params = {
                ...
                message: strMess,
                onConfirm: function (popup) {
                    Framework.Ajax.PostJson(entityobj, 'Pre_UserRoles/Delete', function () {
                        if (results.Data.RespAttachInfo.bError) {
                            //显示错误
                            var strerrors = Framework.UI.Behavior.ErrorHandling(results);
                            if (strerrors.length > 0)
